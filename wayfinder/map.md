...
- [MusicBrainz and Discogs as the Catalog source](tickets/002-research-musicbrainz-catalog.md): 4.49M Albums / 5.75M Editions. Full dump is 7 GB compressed, CC0, ~60–100 GB loaded (4 GB RAM without Solr); genre assignments and ratings are a separate 490 MB BY-NC-SA dump; the hourly live feed is NC too. API is 1 req/s per IP — cache-miss path only; every real consumer runs a replica. Genres are a flat ~2,195-name list, no hierarchy; 25.7% of Albums have any tag, 95.5% have a year, only 5.6% have a rating (mostly single-vote — useless as a proxy). Cover Art Archive serves per-Album art with no rate limit. Discogs counts can't be stored (Restricted); its CC0 dumps add credits and a genre/style taxonomy.
- [Open-data sources for the Popularity Proxy](tickets/005-research-popularity-proxy.md): ListenBrainz's release-group popularity endpoint is the primary (unauthenticated, 1,000 MBIDs/call, listen and listener counts, CC0 dumps); Wikipedia pageviews and Wikidata awards/review-score flags add attention and quality signals, CC0. MusicBrainz ratings are too thin and BY-NC-SA. Spotify, Deezer, Last.fm, Discogs all excluded on terms. Open question: licence of ListenBrainz's popularity numbers (possibly MLHD+, non-commercial).
- [Where critic reviews can be sourced from, per Publication](tickets/004-research-critic-review-sourcing.md): 36 Publications probed. Scores are facts (*Feist*) and public-page scraping isn't a CFAA crime (*hiQ*); the risk is contract, and it's unsettled. Cleanly fetchable with scores: Guardian (API, but 24-hour retention clause), Needle Drop (feed), AllMusic, Line of Best Fit, DIY, musicOMH, Spectrum Culture, Independent (page JSON-LD). Pitchfork renders its score client-side and Condé Nast bans aggregation — hardest big name. Quietus, Stereogum, RA have no scores at all. Flagship names skew unfetchable; a clean list skews UK-indie.
